...
from neural_network.HW_3.cars.utils import Action
from neural_network.HW_3.learning_algorithms.network import Network
...
class SimpleCarAgent(Agent):
    def __init__(self, history_data=int(50000)):
        ...
        self._rays = 13  # выберите число лучей ладара; например, 5
    ...
    def receive_feedback(self, reward, train_every=50, reward_depth=15):
        """
        ...
        if not self.evaluate_mode and (len(self.reward_history) >= train_every) and not (self.step % train_every):
            X_train = np.concatenate([self.sensor_data_history, self.chosen_actions_history], axis=1)[:-14]
            y_train = list(self.reward_history)[:-14]
