...
from neural_network.HW_3.cars.track import plot_map
from neural_network.HW_3.cars.utils import CarState, to_px, rotate, intersect_ray_with_segment, draw_text, angle
...
class SimpleCarWorld(World):
    COLLISION_PENALTY = 100 * 1e0  # выберите сами
    ...
    def set_agents(self, agents=1, agent_class=None):
        ...
        if type(agents) is int:
            ...
            raise ValueError("Parameter agent should be int or proc_list of agents instead of %s" % type(agents))
    ...
    def reward(self, state, collision):
        ...
        collision_penalty = - max(abs(state.velocity), 0.1) * int(collision) * self.COLLISION_PENALTY * (1.2 - abs(a))
    ...
    def evaluate_agent(self, agent, steps=800, visual=True):
        """
